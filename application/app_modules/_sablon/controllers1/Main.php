 	...
 	public function checkStatus(){
  	...
  	function data_rencana_mandor()
  	{
	    ...
	    if($isLogin){
			...
			$data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
			$data["cat"]  = $this->Sablon_Model->getCat();
			$data["catCampur"] = $this->Sablon_Model->getCatCampur();
			$data["minyak"]	= $this->Sablon_Model->getMinyak();
				        ...
				        <div class='col-md-12'>
		  				...
		  				<div class='form-group col-md-4'>
				        	<select class='form-control' name='warna_cat[]' id='warna_cat'>
								...
								$form .=  "<option value='".$c['kd_gd_bahan']."'>".$c['nm_barang']."  (".$c['warna'].")</option>";}
				$form .="	</select>
				        </div>
				        <div class='form-group col-md-3' style='width:217px; padding-right:0px;'>
				        ...
				        <label>Minyak :</label>
				        	<select class='form-control' name='minyak[]' id='minyak' required>
				        		...
				        		foreach ($minyak->result_array() as $m) {
								$form .= "<option value='".$m['kd_gd_bahan']."'>".$m['nm_barang']."</option>";}
				$form .="	</select>
				        </div>
				        <div class='form-group col-md-3' style='width:217px; padding-right:0px;'>
				        <label>&nbsp;</label>
  	...
  	function saveHasilSablon()
  	{
    	...
    	$satuan_cat[$key] == "Kg" ? $jumlah_cat[$key]=$jum_cat[$key] : $jumlah_cat[$key]=$jum_cat[$key]*0.028;
    	$satuan_sisaCat[$key] == "Kg" ? $sisaCat[$key]=$sisa_cat[$key] : $sisaCat[$key]=$sisa_cat[$key]*0.028;
			$cat[] = array(
	    ...
	    $satuan_minyak == "Kg" ? $jumlah_minyak=$jum_minyak : $jumlah_minyak=$jum_minyak*0.028;
    	$satuan_sisaMinyak== "Kg" ? $sisaMinyak=$sisa_minyak : $sisaMinyak=$sisa_minyak*0.028;
    	$minyak = array();
  	...
  	function kirimHasilSablon()
  	{
	    ...
	    if($isLogin){
			...
			if ($result = json_decode($this->Sablon_Model->getDataKirimSablon($tgl), TRUE)) {
				...
				$result = $this->Sablon_Model->kirimHasilSablon($data,$bahan,$tgl);
				echo $result;
			}else{
				echo "Sent";
			}
	...
	function updatePenggunaanBahanSablon()
	{
	    ...
	    if($isLogin){
	    	$satuan_pemakaian = $this->input->post("satuan_pemakaian");
	      	...
	      	$data = array();
	      	$satuan_pemakaian == "Kg" ? $jumPengambilan=$jumlah_pengambilan : $jumPengambilan=$jumlah_pengambilan*0.028;
	      	$satuan_sisa == "Kg" ? $sisaPengambilan=$sisa_pengambilan : $sisaPengambilan=$sisa_pengambilan*0.028;
	      	$data["id"] = $this->input->post("id");
