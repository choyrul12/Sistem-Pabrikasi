...
class Sablon_Model extends CI_Model{
	...
	public function getDataRencanaPIC()
	{
		...
		$this->datatables->where("year(tgl_rencana) = year(now()) and month(now()) and bagian = 'SABLON' and sts_pengerjaan != 'FINISH' and sisa >0");
  ...
  public function getDataSearchRencanaPIC($tgl1,$tgl2)
  {
    $this->datatables->select("*");
    $this->datatables->from("rencana_ppic");
    $this->datatables->where("tgl_rencana between '$tgl1' and '$tgl2' and bagian = 'SABLON' and sts_pengerjaan != 'FINISH' and sisa >0 ");
    $this->db->order_by("tgl_rencana asc");
    ...
    $this->datatables->where("kd_ppic = '$id' and sts_pengerjaan='PENDING'");
  ...
  function insertRencanaSusulan($data)
  {
	...
	{
		$data =  $this->db->query("select a.*, b.satuan_kilo from rencana_sablon a join rencana_ppic b on a.kd_ppic = b.kd_ppic where a.kd_sablon = '$key' and b.deleted ='FALSE'");
		return $data; 
	}
	...
	function getCat()
	{
		$data = $this->db->query("select kd_gd_bahan, nm_barang, warna, status from gudang_bahan where jenis = 'CAT MURNI' ");
		return $data;
	}
	...
	function getMinyak()
	{
		$data = $this->db->query("select kd_gd_bahan, nm_barang, status from gudang_bahan where jenis = 'MINYAK' ");
		return $data;
	}
  ...
  function getDataPenggunaanBahanSablon($tgl)
  {
	...
	function kirimHasilSablon($data,$bahan,$tgl)
	{
		$this->db->trans_begin();
